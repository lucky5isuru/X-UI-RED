...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/lucky5isuru/x-ui-english/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    if [[ $yn =~ "Y"|"y" ]]; then
        ...
        last_version=$(curl -Ls "https://api.github.com/repos/lucky5isuru/x-ui-english/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/lucky5isuru/x-ui-english/main/config/version)
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/lucky5isuru/x-ui-english/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        wget -N --no-check-certificate https://raw.githubusercontent.com/lucky5isuru/x-ui-english/main/x-ui.sh -O /usr/bin/x-ui
...
reset_user() {
    ...
    echo -e "Panel login user name has been reset to: ${RED} ${config_account} ${PLAIN}"
    echo -e "Panel login password has been reset to: ${RED} ${config_password} ${PLAIN}"
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        echo -e "After the setting port is complete. Use the newly set port ${${RED}} ${port} ${PLAIN} to access the panel"
        confirm_restart
    fi
}
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/lucky5isuru/x-ui-english/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_status() {
    ...
    case $? in
        0)
            echo -e "Panel state: ${RED}Running${PLAIN}"
            show_enable_status
        ;;
        1)
...
show_enable_status() {
    check_enabled
    if [[ $? == 0 ]]; then
        echo -e "Whether to start at your own boot: ${RED}Yes${PLAIN}"
    else
...
show_xray_status() {
    check_xray_status
    if [[ $? == 0 ]]; then
        echo -e "XRay status: ${RED}Running${PLAIN}"
    else
...
show_usage() {
    ...
    echo -e ""
    echo -e "${RED} --------------------------------------------------------------------- ${PLAIN}"
    echo -e "${RED}   __   __           _    _ _____    ______             _ _     _      ${PLAIN}"
    echo -e "${RED}   \ \ / /          | |  | |_   _|  |  ____|           | (_)   | |     ${PLAIN}"
    echo -e "${RED}    \ V /   ______  | |  | | | |    | |__   _ __   __ _| |_ ___| |__   ${PLAIN}"
    echo -e "${RED}     > <   |______| | |  | | | |    |  __| |  _ \ / _  | | / __|  _ \  ${PLAIN}"
    echo -e "${RED}    / . \           | |__| |_| |_   | |____| | | | (_| | | \__ \ | | | ${PLAIN}"
    echo -e "${RED}   /_/ \_\           \____/|_____|  |______|_| |_|\__, |_|_|___/_| |_| ${PLAIN}"
    echo -e "${RED}                                                  __/ |                ${PLAIN}"
    echo -e "${RED}                                                 |___/                 ${PLAIN}"
    echo -e "${RED} --------------------------------------------------------------------- ${PLAIN}"
    echo -e ""
    echo -e "------------------------------------------------------------------------------"
...
show_menu() {
    echo -e "
 -------------------------------------------------------------------------------- 
  ${YELLOW}   __   __           _    _ _____    ______             _ _     _       ${PLAIN} 
  ${YELLOW}   \ \ / /          | |  | |_   _|  |  ____|           | (_)   | |      ${PLAIN}
  ${YELLOW}    \ V /   ______  | |  | | | |    | |__   _ __   __ _| |_ ___| |__    ${PLAIN}
  ${YELLOW}     > <   |______| | |  | | | |    |  __| |  _ \ / _  | | / __|  _ \   ${PLAIN}
  ${YELLOW}    / . \           | |__| |_| |_   | |____| | | | (_| | | \__ \ | | |  ${PLAIN} 
  ${YELLOW}   /_/ \_\           \____/|_____|  |______|_| |_|\__, |_|_|___/_| |_|  ${PLAIN}
  ${YELLOW}                                                  __/ |                 ${PLAIN}
  ${YELLOW}                                                 |___/                  ${PLAIN}
--------------------------------------------------------------------------------
  ${RED}X-UI ENGLISH PANEL MANAGEMENT SCRIPT ${PLAIN}
--------------------------------------------------------------------------------
  ${RED}0.${PLAIN} Exit Script
--------------------------------------------------------------------------------
  ${RED}1.${PLAIN} Install X-UI
  ${RED}2.${PLAIN} Update X-UI
  ${RED}3.${PLAIN} Uninstall X-UI
--------------------------------------------------------------------------------
  ${RED}4.${PLAIN} Reset Username Password
  ${RED}5.${PLAIN} Reset Panel Settings
  ${RED}6.${PLAIN} Set the Panel Web Port
--------------------------------------------------------------------------------
  ${RED}7.${PLAIN} Start X-UI
  ${RED}8.${PLAIN} Stop X-UI
  ${RED}9.${PLAIN} Restart X-UI
 ${RED}10.${PLAIN} Check X-UI Status
 ${RED}11.${PLAIN} View X-UI Log
---------------------------------------------------------------------------------
 ${RED}12.${PLAIN} Set the X-UI auto-start at boot
 ${RED}13.${PLAIN} Cancel the X-UI auto-start at boot
---------------------------------------------------------------------------------
 ${RED}14.${PLAIN} Update Geosite and Geoip
 ${RED}15.${PLAIN} One-click installation BBR (the latest kernel)
 ${RED}16.${PLAIN} One-click application certificate (ACME script application)
 ${RED}17.${PLAIN} Open all network ports in the server
 ${RED}18.${PLAIN} Install and configure Cloudflare Warp (Experimental)
 --------------------------------------------------------------------------------   "
    show_status
    echo ""
    if [[ -n $v4 && -z $v6 ]]; then
        echo -e "Panel IPv4 login address is: ${RED}http://$v4:$config_port ${PLAIN}"
    elif [[ -n $v6 && -z $v4 ]]; then
        echo -e "Panel IPv6 login address is: ${RED}http://[$v6]:$config_port ${PLAIN}"
    elif [[ -n $v4 && -n $v6 ]]; then
        echo -e "Panel IPv4 login address is: ${RED}http://$v4:$config_port ${PLAIN}"
        echo -e "Panel IPv6 login address is: ${RED}http://[$v6]:$config_port ${PLAIN}"
    fi
    ...
    case "${num}" in
        ...
        14) update_geo ;;
        15) install_bbr ;;
        16) wget -N --no-check-certificate https://raw.githubusercontent.com/lucky5isuru/x-ui-english/main/acme.sh && bash acme.sh && before_show_menu ;;
        17) open_ports ;;
